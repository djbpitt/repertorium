...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron"
    ...
    <sch:p>Basic rules shared with Zograph project. See also supplementary Repertorium-only rules at
        <sch:emph>tei_ms_repertorium_supplement.sch</sch:emph></sch:p>
    ...
    <sch:pattern id="article-title-rules">
        <sch:rule context="tei:msItemStruct/tei:title">
            <sch:let name="titles" value="doc('../aux/titles.xml')"/>
        ...
        <sch:rule context="tei:msDesc/tei:msIdentifier">
            ...
            <sch:assert test="
                    if (tei:msName[@type eq 'individual']) then
                        tei:msName[@type eq 'individual' and @xml:lang eq 'bg'] and tei:msName[@type eq 'individual' and @xml:lang eq 'en'] and tei:msName[@type eq 'individual' and @xml:lang eq 'ru']
                    else
    ...
    <sch:pattern id="watermark-rules">
        ...
        <sch:rule context="tei:watermark/tei:ref">
            <sch:report
                test="*[1][self::tei:date] or *[last()][self::tei:num] or tei:num/following-sibling::*[1][not(self::tei:date)] or tei:date/following-sibling::*[1][not(self::tei:num)]"
    ...
    <sch:pattern id="misc-rules">
        ...
        <sch:rule context="tei:extent/tei:dimensions">
            <sch:assert test="@type = ('folia', 'written')">extent/dimensions must specify a @type
                attribute of either 'folia' or 'written'</sch:assert>
        </sch:rule>
    </sch:pattern>
    ...
    <sch:pattern id="orthography-rules">
        ...
        <sch:rule context="orthNote[@type eq 'jus']">
            <sch:assert
                test="@subtype = ('etymReg', 'nonEtymReg', 'nonConsis', 'nonJus', 'jusTrace')">Legal
                values for jus @subtype are 'etymReg', 'nonEtymReg', 'nonConsis', 'nonJus', and
                'jusTrace'.</sch:assert>
        </sch:rule>
