    ...
    <xsl:template name="xsl:initial-template">
        <html>
            ...
            <body>
                ...
                <p>Left: <cite>Draganov menaion</cite>; right: <cite>Sinai 25</cite>; <span
                        style="background-color: lightgreen;">green = agreement</span>; <span
                        style="background-color: pink;">red = disagreement</span>; <xsl:value-of
                ...
                <xsl:for-each select="$all-dates => re:sort-gMonthDay()">
                    ...
                    <section>
                        <xsl:attribute name="class"
                            select="
                                'date ' || (if (count(($dragEntry, $sinEntry)) lt 2)
                                then
                                    ()
                                else
                                    if
