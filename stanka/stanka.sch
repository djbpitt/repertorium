...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2"
    ...
    <sch:pattern>
        <sch:rule context="tei:msItemStruct/tei:title">
            <sch:assert test="@xml:lang eq 'bg'">Titles of articles must specify an @xml:lang
                attribute with the value 'bg'.</sch:assert>
            <sch:report
                test="
                    some $i in text()
        ...
        <sch:rule context="tei:msItemStruct">
            <sch:assert test="starts-with(@xml:id, 'ACD')">&lt;msItemStruct&gt; elements must have
                an @xml:id value that begins with the string 'ACD'.</sch:assert>
        </sch:rule>
